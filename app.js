(function(){
  ...
  // Nombre de visites confirmées à partir duquel un lieu connu est "de confiance" et déclenche
  // la création automatique d'un bloc agenda (sinon, retour au flux de confirmation habituel).
  // Placée ici (avant loadData(), pas dans la zone géoloc plus bas) car la migration de
  // loadData() en a besoin dès l'ouverture — une déclaration plus bas dans le fichier serait
  // encore à 'undefined' au moment où loadData() s'exécute (var hoisté, pas sa valeur).
  var TRUSTED_VISIT_THRESHOLD = 5;
  function defaultState(){
    return {
      ...
      activityTargetMinutes: 150, // objectif hebdomadaire d'activité physique, réglable dans Suivi santé
  ...
  function loadData(){
    ...
    if(!result){
      // Première ouverture : pas de données locales -> état vierge.
      result = defaultState();
    }
    ...
    if(typeof result.activityTargetMinutes === 'undefined') result.activityTargetMinutes = 150;
    ...
    if(!result.recurring) result.recurring = [];
    if(!result.recurringLog) result.recurringLog = {};
    ...
    // MIGRATION 1 — knownLocations sans visitCount -> TRUSTED_VISIT_THRESHOLD, pas 0 : ces lieux
    // sont déjà utilisés/fiables aujourd'hui (créés avant l'existence du seuil de confiance),
    // il ne faut pas les faire retomber soudainement dans le flux de confirmation.
    if(result.knownLocations) result.knownLocations.forEach(function(loc){
      if(typeof loc.visitCount !== 'number') loc.visitCount = TRUSTED_VISIT_THRESHOLD;
    });
    // MIGRATION 2 — recurring sans frequency -> {count:1, unit:'day'} (comportement quotidien implicite
    // déjà en vigueur avant l'existence de ce champ, donc pas un changement de comportement observable)
    result.recurring.forEach(function(r){
      if(!r.frequency) r.frequency = {count:1, unit:'day'};
    });
    return result;
  }
  ...
  function applyRemoteData(remote, remoteTs) {
    var lastSyncTs = parseInt(localStorage.getItem(SYNC_TS_KEY) || '0');
    if (remoteTs <= lastSyncTs) return; // pas plus récent, rien à faire
    if (!remote.tasks && !remote.goals && !remote.blocks && !remote.recurring) return; // state vide
    localStorage.setItem(STORE_KEY, JSON.stringify(remote));
    localStorage.setItem(SYNC_TS_KEY, remoteTs.toString());
  ...
  function syncFromSupabase() {
    ...
    // Re-vérifie toutes les 2 minutes si des changements viennent d'un autre appareil
  ...
  // Point d'entrée unique pour la clé de "jour routine" : avant 6h, rattachée à la veille (une routine
  // cochée à 1h du matin compte pour la journée en cours, pas pour le nouveau jour civil). Jamais
  // recalculée inline ailleurs.
  function routineDayKey(date) {
    var d = new Date(date);
  ...
  function quickCaptureAdd(text){
    ...
    if(time && dayOffset!==null){
      ...
      var grp=state.tasks.filter(function(t){return t.urgency==='week'&&t.status!=='done';});
      var maxP=grp.reduce(function(m,t){return Math.max(m,t.priority!==undefined?t.priority:0);},- 1);
      state.tasks.push({
        id:'t'+Date.now()+Math.floor(Math.random()*1000),
        text:text, urgency:'week', status:'todo', pillarId:chosenPillar,
        deadline:null, durationMinutes:null, priority:maxP+1, doneAt:null, createdAt:new Date().toISOString()
      });
      saveData(); captureInput.value=''; capturePillarOverride=null;
      capturePreview.style.display='none';
      renderNow();
  ...
  sendBtn.addEventListener('click', function(){ quickCaptureAdd(captureInput.value); });
  ...
  captureInput.addEventListener('keydown', function(e){ if(e.key==='Enter') quickCaptureAdd(captureInput.value); });
  ...
  function renderNowBoussole(){
    ...
    var scoreResult = computeBoussoleScore();
    var ideal = scoreResult.ideal;
    var realResult = scoreResult.realResult;
    var realSplit = scoreResult.real;
    var alignScore = scoreResult.alignScore;
    var boussolScore = scoreResult.boussolScore;
  ...
  function renderAgenda(){
    ...
    // ---- Section Nuit : blocs de la veille post-minuit + blocs du jour entre 0h et 6h ----
    ...
    var nuitBlocks = prevDayBlocks.map(function(b){
      return Object.assign({}, b, {
        ...
        _nuitLabel: pad2(b.startHour - 24) + 'h' + pad2(b.startMinute || 0),
        _fromPrevDay: true
      });
    }).filter(function(b){ return b._nuitMinute < 6 * 60; });
    // Blocs du jour courant qui commencent avant 6h (géo détecté la nuit, coucher manuel, etc.)
    var todayEarlyNuit = dayBlocks.filter(function(b){ return blockStartMinutes(b) < 6 * 60; }).map(function(b){
      return Object.assign({}, b, {
        _nuitMinute: blockStartMinutes(b),
        _nuitLabel: pad2(b.startHour) + 'h' + pad2(b.startMinute || 0),
        _fromPrevDay: false
      });
    });
    nuitBlocks = nuitBlocks.concat(todayEarlyNuit).sort(function(a,b){ return a._nuitMinute - b._nuitMinute; });
    ...
    var slotsHtml = periodOrder.map(function(periodKey){
      ...
      if (periodKey === 'nuit') {
        ...
        var nuitRowsHtml = nuitBlocks.map(function(block){
          var col = pillarColor(block.pillarId);
          var hPx = 28;
          return '<div class="hour-slot" style="min-height:32px;">'+
            '<div class="block" data-blockid="'+block.id+'" style="background:'+col+'22;border:1px solid '+col+';border-left:3px solid '+col+';height:'+hPx+'px;left:4px;right:4px;width:auto;">'+
              '<div class="block-main">'+
                '<div class="block-title" style="color:'+col+';"><i class="ti ti-moon" style="font-size:11px;margin-right:3px;opacity:0.7;"></i>'+escapeHtml(block.title)+'<span style="opacity:0.45;font-size:10px;margin-left:5px;">'+block._nuitLabel+'</span></div>'+
              '</div>'+
              '<button class="block-del-btn" data-blockdel="'+block.id+'" style="background:none;border:none;cursor:pointer;padding:2px 4px;color:'+col+';opacity:0.55;flex-shrink:0;" title="Supprimer">'+svgIcon('trash',11)+'</button>'+
            '</div>'+
          '</div>';
        }).join('');
  ...
  // ---- Drag and drop tactile/souris : bloc existant -> nouveau créneau ----
  ...
  function startBlockDrag(e, source, grid){
    ...
    if(!b) return;
    var label = b.title, col = pillarColor(b.pillarId);
    dragActive = true;
    ...
    function onUp(ev){
      ...
      if(targetSlot){
        ...
        moveBlockToSlot(source.id, startMin);
      }
    }
  ...
  var HEALTH_DIMS = [
    {id:'sommeil',label:'Sommeil',icon:'ti-moon',color:'#818CF8',auto:true},
    {id:'activite',label:'Activité physique',icon:'ti-run',color:'#34D399',auto:true},
    {id:'nutrition',label:'Nutrition',icon:'ti-salad',color:'#F97316',auto:false},
    {id:'prevention',label:'Suivi médical',icon:'ti-stethoscope',color:'#EC4899',auto:false}
  ];

  // Calcule automatiquement les dimensions "sommeil" et "activité" depuis le journal réel
  // (sleepQualityLog / activityLog des 7 derniers jours). Si aucune donnée sur la période,
  // conserve la dernière valeur connue plutôt que de faire chuter le score à 0.
  function computeHealthScores(){
    if(!state.healthScores) state.healthScores = {sommeil:70,activite:70,nutrition:70,prevention:70};
    var hs = state.healthScores;
    var last7Keys = [];
    for(var i=0;i<7;i++){ var d=new Date(); d.setDate(d.getDate()-i); last7Keys.push(dateKey(d)); }

    // Sommeil : moyenne des qualités notées (1-5 → ×20) sur les 7 derniers jours
    var qualityLog = state.sleepQualityLog || {};
    var qualities = last7Keys.map(function(k){ return qualityLog[k]; }).filter(function(q){ return typeof q === 'number'; });
    if(qualities.length){
      hs.sommeil = Math.round((qualities.reduce(function(s,q){ return s+q; },0)/qualities.length)*20);
    }

    // Activité : minutes loggées sur 7 jours vs objectif hebdomadaire
    var last7Set = {}; last7Keys.forEach(function(k){ last7Set[k]=true; });
    var recentActivity = (state.activityLog||[]).filter(function(a){ return last7Set[a.date]; });
    if(recentActivity.length){
      var totalMin = recentActivity.reduce(function(s,a){ return s+(a.durationMinutes||0); },0);
      var target = state.activityTargetMinutes || 150;
      hs.activite = Math.min(100, Math.round((totalMin/target)*100));
    }
  ...
  // Garde state.allocations synchronisé sur le profil actif (utilisé par les suggestions de tendance
  // 14 jours, renderGapObjectives/renderSuggestion) — appelé à chaque changement de profil actif,
  // pas seulement à la fin du wizard, pour éviter qu'il reste figé sur un profil obsolète.
  function syncAllocationsFromActiveProfile(){
    var pcts=profileIdealPercents(activeProfile())||{};
    if(!state.allocations) state.allocations={};
    PILLARS_ORDER.forEach(function(p){ state.allocations[p]=pcts[p]||0; });
  }
  ...
  // L'idéal d'un profil vient en priorité de sa "semaine de référence" (une semaine réellement
  // vécue et jugée représentative), sinon on retombe sur la configuration manuelle du wizard.
  function profileIdealPercents(profile){
    if(profile.referenceWeekMonday){
      var ref=computeSplitForWeek(new Date(profile.referenceWeekMonday));
      if(ref) return ref.split;
    }
    return hoursToPercents(profileHoursPerWeek(profile));
  }

  function formatWeekRangeLabel(monday){
    var sunday=sundayOf(monday);
    var fmt=function(d){ return pad2(d.getDate())+'/'+pad2(d.getMonth()+1); };
    return fmt(monday)+' → '+fmt(sunday);
  }

  // ---- Modale de sélection de la semaine de référence ----
  function openReferenceWeekModal(profile){
    var modal=document.getElementById('modal-reference-week');
    var currentWrap=document.getElementById('reference-week-current');
    var currentLabel=document.getElementById('reference-week-current-label');
    var clearBtn=document.getElementById('reference-week-clear');
    var listEl=document.getElementById('reference-week-list');
    if(!modal||!listEl) return;

    if(profile.referenceWeekMonday){
      currentWrap.style.display='block';
      currentLabel.textContent='Référence actuelle : semaine du '+formatWeekRangeLabel(new Date(profile.referenceWeekMonday));
      clearBtn.onclick=function(){
        delete profile.referenceWeekMonday;
        saveData();
        closeReferenceWeekModal();
        renderBalance();
      };
    } else {
    ...
    // ~26 dernières semaines complètes avec assez de données (la semaine en cours n'est pas encore terminée)
    var rows=[];
    var cursor=mondayOf(new Date());
    cursor.setDate(cursor.getDate()-7);
    for(var i=0;i<26;i++){
      var split=computeSplitForWeek(cursor);
      if(split) rows.push({monday:new Date(cursor), split:split.split});
      cursor.setDate(cursor.getDate()-7);
    }

    if(!rows.length){
      listEl.innerHTML='<div class="empty-state"><i class="ti ti-calendar-off"></i><p>Pas encore assez de semaines avec des données pour en choisir une comme référence.</p></div>';
    } else {
      listEl.innerHTML=rows.map(function(r){
        var mk=dateKey(r.monday);
        var isCurrent=profile.referenceWeekMonday===mk;
        var chips=PILLARS_ORDER.map(function(pid){
          var pct=r.split[pid]||0; if(!pct) return '';
          return '<span style="font-size:10.5px;color:'+PILLAR_COLORS[pid]+';">'+pct+'%</span>';
        }).filter(function(s){return s;}).join(' · ');
        return '<div class="goal-card" style="display:flex;align-items:center;justify-content:space-between;gap:8px;">'+
          '<div>'+
            '<div style="font-size:12.5px;color:var(--ink);font-weight:600;">'+formatWeekRangeLabel(r.monday)+'</div>'+
            '<div style="margin-top:2px;">'+chips+'</div>'+
          '</div>'+
          '<button class="btn '+(isCurrent?'primary':'')+' sm" data-refweek="'+mk+'">'+(isCurrent?'Choisie':'Choisir')+'</button>'+
        '</div>';
      }).join('');
      listEl.querySelectorAll('[data-refweek]').forEach(function(btn){
        btn.addEventListener('click',function(){
          profile.referenceWeekMonday=btn.dataset.refweek;
          saveData();
          closeReferenceWeekModal();
          renderBalance();
        });
  ...
  function closeReferenceWeekModal(){
    var modal=document.getElementById('modal-reference-week');
    if(modal) modal.style.display='none';
  }

  document.getElementById('reference-week-close').addEventListener('click', closeReferenceWeekModal);
  document.getElementById('modal-reference-week').addEventListener('click', function(e){ if(e.target===this) closeReferenceWeekModal(); });
  ...
  function renderHealthSliders(){
    ...
    var hs=computeHealthScores();
    wrap.innerHTML=HEALTH_DIMS.map(function(d){
      var val=hs[d.id]!==undefined?hs[d.id]:70;
      var autoNote=d.auto?'<div style="font-size:10.5px;color:var(--ink-faint);margin-top:3px;">Calculé automatiquement depuis ton journal</div>':'';
      var activityTargetField=(d.id==='activite')?(
        '<div style="display:flex;align-items:center;gap:6px;margin-top:5px;">'+
          '<label style="font-size:10.5px;color:var(--ink-faint);" for="activity-target-input">Objectif hebdo</label>'+
          '<input type="number" id="activity-target-input" min="10" step="10" value="'+(state.activityTargetMinutes||150)+'" style="width:64px;font-size:11px;padding:3px 6px;background:var(--bg-input);border:1px solid var(--card-border);border-radius:6px;color:var(--ink);">'+
          '<span style="font-size:10.5px;color:var(--ink-faint);">min/sem</span>'+
        '</div>'
      ):'';
      return '<div style="margin-bottom:12px;">'+
        ...
        '<input type="range" id="hs-'+d.id+'" min="0" max="100" value="'+val+'"'+(d.auto?' disabled':'')+' style="width:100%;accent-color:'+d.color+';cursor:'+(d.auto?'default':'pointer')+';height:4px;'+(d.auto?'opacity:0.6;':'')+'">'+
        autoNote+activityTargetField+
      '</div>';
    }).join('');
    ...
    var targetInput=document.getElementById('activity-target-input');
    if(targetInput){
      targetInput.addEventListener('change',function(){
        var v=parseInt(this.value);
        state.activityTargetMinutes=(v>0)?v:150;
  ...
  function renderWizardStep(){
    ...
    if(!isSummary){
      ...
      document.getElementById('wiz-save').addEventListener('click',function(){
        ...
        syncAllocationsFromActiveProfile();
  ...
  function renderBalance(){
    ...
    if(tabsEl){
      ...
      tabsEl.querySelectorAll('.profile-tab').forEach(function(btn){
        btn.addEventListener('click',function(){state.activeProfileId=btn.dataset.pid;syncAllocationsFromActiveProfile();saveData();renderBalance();});
      });
    }
    var cfgBtn=document.getElementById('configure-profile-btn');
    if(cfgBtn){ cfgBtn.innerHTML='<i class="ti ti-settings"></i> Configurer'; cfgBtn.onclick=function(){startWizard(profile);}; }
    ...
    var refWeekBtn=document.getElementById('reference-week-btn');
    if(refWeekBtn){ refWeekBtn.onclick=function(){ openReferenceWeekModal(profile); }; }

    // Camemberts — le camembert "idéal" reflète le profil consulté via les onglets (navigation libre)
    var pcts=profileIdealPercents(profile);
    ...
    var refNoteEl=document.getElementById('reference-week-note');
    if(refNoteEl) refNoteEl.textContent=profile.referenceWeekMonday?'Basé sur ta semaine du '+formatWeekRangeLabel(new Date(profile.referenceWeekMonday)):'';
    ...
    // Score — toujours basé sur le profil réellement assigné à la semaine courante (peut différer
    // de l'onglet consulté ci-dessus), pour rester cohérent entre vue Maintenant et vue Équilibre.
    var scoreResult=computeBoussoleScore();
    var ideal=scoreResult.ideal, alignScore=scoreResult.alignScore, healthScore=scoreResult.healthScore, boussolScore=scoreResult.boussolScore;
    ...
    if(sd){
      var scoreDetailTxt=boussolScore!==null?'Alignement '+alignScore+'/100 · Santé '+healthScore+'/100':'Configure ton profil et pose des blocs cette semaine.';
      if(boussolScore!==null && scoreResult.profile.id!==profile.id){
        scoreDetailTxt+=' · Score calculé avec le profil "'+escapeHtml(scoreResult.profile.name)+'" (assigné à cette semaine)';
      }
      sd.textContent=scoreDetailTxt;
    }

    // Écarts (toujours vs le profil de score, cohérent avec le chiffre ci-dessus)
    var ge=document.getElementById('balance-gaps');
    if(ge){
      if(realResult&&scoreResult.hasProfile){
        ...
        ge.innerHTML='<div style="text-align:center;padding:8px 0;">'+
          '<p style="font-size:12.5px;color:var(--ink-faint);margin:0 0 10px;">'+(!scoreResult.hasProfile?"Configure d'abord ton profil ci-dessous.":'Pose des blocs cette semaine pour voir les écarts.')+'</p>'+
          (!scoreResult.hasProfile?'<button class="btn sm" id="cfg-first-btn"><i class="ti ti-settings"></i> Configurer mon profil</button>':'')+
        '</div>';
        if(!scoreResult.hasProfile){ var cfb=document.getElementById('cfg-first-btn'); if(cfb) cfb.onclick=function(){startWizard(scoreResult.profile);}; }
      }
    }
    ...
    // Santé
    var hs=scoreResult.healthDims;
    renderHealthSliders();
    var shb=document.getElementById('health-save-btn');
    if(shb){ shb.onclick=function(){
      if(!state.healthScores) state.healthScores={};
      HEALTH_DIMS.forEach(function(d){ if(!d.auto) state.healthScores[d.id]=parseInt(document.getElementById('hs-'+d.id).value)||70; });
      saveData();renderBalance();renderNow();
  ...
  // Répartition réelle par pilier sur un intervalle de dates [startKey, endKey] inclus (hors sommeil/déjeuner)
  function computeSplitForRange(startKey,endKey){
    var minutesByPillar={};
    state.blocks.forEach(function(b){
      if(!b.date||b.isSleepBlock||b.isLunchBlock)return;
      if(b.date<startKey||b.date>endKey)return;
  ...
  function computeRealSplit(){
    ...
    return computeSplitForRange(dateKey(monday),todayKey);
  }

  // Répartition réelle sur une semaine complète passée (lundi → dimanche), utilisée pour dériver
  // l'idéal d'un profil depuis une "semaine de référence" réellement vécue.
  function computeSplitForWeek(monday){
    return computeSplitForRange(dateKey(monday),dateKey(sundayOf(monday)));
  }
  ...
  // Le profil "idéal" d'une semaine suit l'assignation faite dans la vue Année
  // (state.weekTypeAssignments) ; à défaut, retombe sur le profil actif sélectionné à la main.
  function resolveWeekProfile(monday){
    ensureProfiles();
    var assignedId = (state.weekTypeAssignments||{})[dateKey(monday)];
    var assigned = assignedId ? state.weekProfiles.find(function(p){ return p.id===assignedId; }) : null;
    return assigned || activeProfile();
  }

  // Point d'entrée unique du Score Boussole — remplace les calculs dupliqués qui existaient
  // séparément dans la vue Maintenant et la vue Équilibre (source de désynchronisation).
  function computeBoussoleScore(){
    var monday = mondayOf(new Date());
    var profile = resolveWeekProfile(monday);
    var pcts = profileIdealPercents(profile);
    ...
    var healthDims = computeHealthScores();
    var healthScore = Math.round(HEALTH_DIMS.reduce(function(s,d){ return s+(healthDims[d.id]||70); },0)/HEALTH_DIMS.length);
    var boussolScore = alignScore!==null ? Math.round(0.6*alignScore + 0.4*healthScore) : null;
    return {
      profile: profile, hasProfile: !!pcts,
      ideal: ideal, real: realSplit, realResult: realResult,
      alignScore: alignScore, healthScore: healthScore, healthDims: healthDims,
      boussolScore: boussolScore
    };
  }
  ...
  var URGENCY_LABELS = {
    ...
    'note':  { label: '📝 Notes',      cls: 'urgency-note',   order: 6, color: '#94A3B8', short: '📝 Note' }
  };
  var _taskUrg = 'week';
  ...
  function renderTasks(){
    ...
    var html='';
    var groups={},gOrder=['hour','day','2days','week','month','custom','note'];
  ...
  // ============ ROUTINES (section dans view-tasks) ============
  var routineModalBackdrop = document.getElementById('routine-modal-backdrop');
  var routineTextInput = document.getElementById('routine-text-input');
  var routineFreqCount = document.getElementById('routine-freq-count');
  var addRoutineBtn = document.getElementById('add-routine-btn');

  function openRoutineModal(){
    routineTextInput.value = '';
    routineFreqCount.value = 1;
    routineModalBackdrop.style.display = 'flex';
    routineTextInput.focus();
  }
  function closeRoutineModal(){
    routineModalBackdrop.style.display = 'none';
  }
  addRoutineBtn.addEventListener('click', openRoutineModal);
  document.getElementById('routine-cancel-btn').addEventListener('click', closeRoutineModal);
  // Clic sur le fond (en dehors de la carte) : ferme, comme le bouton Annuler.
  routineModalBackdrop.addEventListener('click', function(e){
    if(e.target === routineModalBackdrop) closeRoutineModal();
  });
  document.addEventListener('keydown', function(e){
    if(e.key==='Escape' && routineModalBackdrop.style.display !== 'none') closeRoutineModal();
  });

  function saveRoutine(){
    var text = routineTextInput.value.trim();
    if(!text) return;
    var det = detectPillar(text);
    var countRaw = parseInt(routineFreqCount.value);
    var unitVal = document.getElementById('routine-freq-unit').value || 'day';
    state.recurring.push({
      id: generateId('r'),
      text: text,
      pillarId: det.pillar || 'personnel',
      type: 'libre',
      frequency: {count: (isNaN(countRaw)||countRaw<1) ? 1 : countRaw, unit: unitVal}
    });
    saveData();
    closeRoutineModal();
    renderRoutinesSection();
  ...
  // Liste "à faire aujourd'hui" : routines dont l'id n'est pas encore dans le log du jour routine
  // courant. Pas de badge d'urgence, pas de tri par priorité — juste une liste à cocher.
  function renderRoutinesSection(){
    var wrap = document.getElementById('routines-section');
    if(!wrap) return;
    var key = routineDayKey(Date.now());
    var log = state.recurringLog[key] || [];
    var todo = state.recurring.filter(function(r){ return log.indexOf(r.id)===-1; });
    if(!todo.length){
      wrap.innerHTML = '';
      return;
    }
    var html = '<div class="tasks-group-label" style="margin-top:24px;">Routines</div>';
    html += todo.map(function(r){
      return '<div class="rec-item" data-routineid="'+r.id+'">'+
        '<div class="rec-check" data-checkroutine="'+r.id+'"></div>'+
        '<div style="flex:1;">'+escapeHtml(r.text)+'</div>'+
        '<div class="item-del" data-delroutine="'+r.id+'">'+svgIcon('trash',16)+'</div>'+
      '</div>';
    }).join('');
    wrap.innerHTML = html;
    wrap.querySelectorAll('[data-checkroutine]').forEach(function(el){
      el.addEventListener('click', function(){
        var id = el.dataset.checkroutine;
        var k = routineDayKey(Date.now());
        if(!state.recurringLog[k]) state.recurringLog[k] = [];
        if(state.recurringLog[k].indexOf(id)===-1) state.recurringLog[k].push(id);
        saveData();
        renderRoutinesSection();
      });
    });
    wrap.querySelectorAll('[data-delroutine]').forEach(function(el){
      el.addEventListener('click', function(e){
        e.stopPropagation();
        var r = findItem(state.recurring, {id: el.dataset.delroutine});
        deleteWithUndo(function(){ return state.recurring; }, el.dataset.delroutine, r?r.text:'routine', [renderRoutinesSection]);
      });
    });
  }
        ...
        return '<div class="acc-card" style="border-left:3px solid '+color+';">'+
          '<div class="acc-card-top">'+
            ...
            '<div style="display:flex;gap:8px;align-items:center;">'+
              '<button class="btn sm" data-acc-import="'+acc.id+'" style="font-size:12px;padding:5px 10px;">📂 Importer relevé</button>'+
  ...
  function parsePDFFile(file, onDone){
    loadScript(_PDFJS_URL, _PDFJS_SRI, function(lerr){
      ...
      try{ pdfjsLib.GlobalWorkerOptions.workerSrc=_PDFJS_WORKER; }catch(e){}
      var reader=new FileReader();
      reader.onload=function(ev){
        ...
        var msg='Impossible de lire ce PDF : '+e.message;
        if(location.protocol==='file:') msg='PDFs non supportés en local. Ouvrez l\'application depuis GitHub Pages pour les PDFs. Excel (.xlsx) et CSV fonctionnent en local.';
        onDone(null,msg);
      });
    };
  ...
  var _geo = { anchorLat: null, anchorLng: null, anchorTime: null, prompted: false, lastConfirmedTime: null };
  // Au-delà de ce trou entre deux positions GPS confirmées, on ne peut plus garantir que
  // l'utilisateur est resté au même endroit — voir _fillLocationGap().
  var GEO_GAP_THRESHOLD_MS = 25 * 60000;
  ...
  var _geoEmptyBtn   = document.getElementById('geo-empty-btn');
  var _geoModalMode  = 'live'; // 'live' = lieu détecté en direct · 'gap' = trou de suivi passé à qualifier
  var _geoPendingGap = null;   // {start, end, suggestedPillar} en attente de réponse — voir _fillLocationGap()
  ...
  var GEO_ACCURACY_IGNORE_THRESHOLD = 500; // mètres, position trop imprécise pour être utilisée
  var GEO_MOVE_ACCURACY_MARGIN = 1.5;      // un déplacement n'est retenu que s'il dépasse accuracy * ce facteur
  ...
  function saveGeoAnchor() {
    try { localStorage.setItem(GEO_ANCHOR_KEY, JSON.stringify({ anchorLat: _geo.anchorLat, anchorLng: _geo.anchorLng, anchorTime: _geo.anchorTime, promptedDate: _geo.prompted ? dateKey(new Date()) : '', ongoingBlockId: _ongoingGeoBlockId, lastConfirmedTime: _geo.lastConfirmedTime })); } catch(e) {}
  }
  function loadGeoAnchor() {
    try {
      var s = JSON.parse(localStorage.getItem(GEO_ANCHOR_KEY));
      // anchorLat peut être null juste après un trou de suivi comblé (_fillLocationGap) :
      // ne pas se baser uniquement dessus pour savoir si un état a été persisté.
      if (s && (s.anchorLat != null || s.lastConfirmedTime)) {
        _geo.anchorLat = (s.anchorLat != null) ? s.anchorLat : null;
        _geo.anchorLng = (s.anchorLng != null) ? s.anchorLng : null;
        _geo.anchorTime = s.anchorTime;
        _geo.lastConfirmedTime = s.lastConfirmedTime || null;
  ...
  // Étend la durée du bloc géo en cours jusqu'à la dernière position GPS confirmée
  // (appelé à chaque check GPS et à l'ouverture). BUG FIX : on n'étend plus jusqu'à
  // Date.now() — sinon un trou de suivi (app fermée/arrière-plan, aucune position reçue)
  // était silencieusement compté comme "toujours sur place" jusqu'à la réouverture,
  // même si l'utilisateur avait quitté le lieu entre-temps. On plafonne désormais à
  // _geo.lastConfirmedTime, la dernière fois qu'une position a réellement été reçue ;
  // le trou lui-même est qualifié séparément par _fillLocationGap() + le bandeau "gap".
  function _extendOngoingBlock() {
    if (_geoPendingGap) return; // un trou est en attente de réponse : ne rien étirer avant qualification
    ...
    var extendUntil = _geo.lastConfirmedTime || Date.now();
    if (extendUntil < bStartDate.getTime()) extendUntil = bStartDate.getTime();
    var newDur = Math.round((extendUntil - bStartDate.getTime()) / 60000);
  ...
  // Détecte un trou de suivi (app fermée, arrière-plan, hors ligne) et, au lieu de laisser
  // _extendOngoingBlock() attribuer silencieusement ce temps au dernier lieu connu, ouvre le
  // bandeau géo en mode "gap" pour demander à l'utilisateur ce qu'il faut en faire (nom + pilier,
  // ou "inconnu/vide"). Appelée à chaque position GPS reçue. Renvoie true si un trou vient
  // d'être détecté (le reste du traitement de cette position est alors différé au prochain check).
  function _fillLocationGap(now) {
    var last = _geo.lastConfirmedTime;
    if (!last || (now - last) < GEO_GAP_THRESHOLD_MS) {
      _geo.lastConfirmedTime = now;
      saveGeoAnchor();
      return false;
    }
    // Récupérer le bloc en cours AVANT de l'étirer, pour pré-remplir le bandeau avec son nom/pilier
    var closedBlock = _ongoingGeoBlockId ? findItem(state.blocks, {id: _ongoingGeoBlockId}) : null;
    var suggestedName = closedBlock ? closedBlock.title : '';
    var suggestedPillar = closedBlock ? closedBlock.pillarId : 'personnel';
    // Clôturer le bloc en cours à la dernière confirmation réelle (AVANT d'écraser
    // lastConfirmedTime, sinon _extendOngoingBlock() étirerait jusqu'à maintenant et on
    // retomberait dans le bug initial du bloc étiré silencieusement)
    _extendOngoingBlock();
    _geo.lastConfirmedTime = now;
    _ongoingGeoBlockId = null;
    saveGeoAnchor();

    _geoPendingGap = { start: last, end: now, suggestedPillar: suggestedPillar };
    var gapStart = new Date(last);
    var gapMin = Math.round((now - last) / 60000);
    var gapH = Math.floor(gapMin / 60), gapM = gapMin % 60;
    var gapLabel = (gapH ? gapH + 'h ' : '') + gapM + 'min';
    openGeoModal(null, null, suggestedName, {
      mode: 'gap',
      hint: 'Position inconnue de ' + pad2(gapStart.getHours()) + 'h' + pad2(gapStart.getMinutes()) + ' à maintenant (' + gapLabel + ')'
    });
    sendNotif('❓ Position inconnue', 'Aucune localisation depuis ' + pad2(gapStart.getHours()) + 'h' + pad2(gapStart.getMinutes()) + ' (' + gapLabel + '). Où étais-tu ?');
    return true;
  }

  // Crée le bloc rétroactif correspondant au trou de suivi une fois qualifié par l'utilisateur
  // (nom + pilier confirmés, ou "inconnu/vide" — voir les handlers de geo-confirm-btn/geo-empty-btn).
  function createGapBlock(name, pillar) {
    if (!_geoPendingGap) return;
    var startDate = new Date(_geoPendingGap.start);
    var durMin = Math.max(1, Math.round((_geoPendingGap.end - _geoPendingGap.start) / 60000));
    state.blocks.push({
      id: 'b_geogap_' + Date.now(),
      pillarId: pillar, title: name,
      date: dateKey(startDate), endDate: null,
      startHour: startDate.getHours(), startMinute: startDate.getMinutes(),
      durationMinutes: durMin, goalId: null, isSleepBlock: false, isLunchBlock: false,
      notes: 'Confirmé rétroactivement après un trou de suivi de localisation.'
    });
    saveData();
    if (!document.hidden) renderAgenda();
  }

  // Résout le trou en attente sans nom précis : pose un bloc neutre "Position inconnue" qui
  // occupe quand même le créneau (pour ne jamais laisser un trou vide dans la journée), puis
  // libère la détection en direct pour le prochain check GPS.
  function resolveGapAsUnknown() {
    if (_geoPendingGap) {
      var startDate = new Date(_geoPendingGap.start);
      var durMin = Math.max(1, Math.round((_geoPendingGap.end - _geoPendingGap.start) / 60000));
      state.blocks.push({
        ...
        startHour: startDate.getHours(), startMinute: startDate.getMinutes(),
        durationMinutes: durMin, goalId: null, isSleepBlock: false, isLunchBlock: false,
        notes: 'Aucun signal de localisation pendant cette période (app fermée ou hors ligne).'
      });
      saveData();
      if (!document.hidden) renderAgenda();
    }
    _geoPendingGap = null;
    _geo.prompted = false; // libère la détection en direct : la position actuelle sera redemandée au prochain check
    saveGeoAnchor();
  }
  ...
  // ---- Création bloc ----
  function createGeoBlock(name, pillarId, locId) {
    ...
    state.blocks.push({
      id: newId, pillarId: pillarId, title: name,
      date: dateKey(startDate), endDate: null,
      startHour: startDate.getHours(), startMinute: startDate.getMinutes(),
      durationMinutes: duration, goalId: null, isSleepBlock: false, isLunchBlock: false, notes: ''
    });
    _geo.prompted = true;
    saveGeoAnchor();
    if (locId) {
      var loc = findItem(state.knownLocations, {id: locId});
      if (loc) loc.visitCount = (loc.visitCount || 0) + 1;
    }
    saveData();
    renderAgenda();
    switchToView('today');
  ...
  // ---- Modal ----
  // opts.mode: 'live' (défaut) = lieu détecté en direct, crée un bloc depuis l'ancre jusqu'à
  // maintenant. 'gap' = trou de suivi passé à qualifier (voir _fillLocationGap) : le champ
  // pilier est pré-rempli avec celui du bloc précédent et le bouton "Inconnu/vide" apparaît.
  function openGeoModal(lat, lng, suggestedName, opts) {
    opts = opts || {};
    _geoModalMode = opts.mode === 'gap' ? 'gap' : 'live';
    _pendingGeoLat = lat; _pendingGeoLng = lng;
    _geoNameInput.value = suggestedName || '';
    // Toujours réafficher les champs normaux : onGeoError() les masque pour l'alerte
    // "permission refusée", qui réutilise ce même bandeau.
    _geoNameInput.style.display = '';
    _geoPillarSel.style.display = '';
    document.getElementById('geo-confirm-btn').style.display = '';
    if (_geoModalMode === 'gap') {
      _geoPlaceHint.textContent = 'Trou de localisation';
      _geoTimeHint.textContent = opts.hint || '';
      _geoEmptyBtn.style.display = '';
      _geoSaveCheck.parentElement.style.display = 'none'; // pas de coordonnées connues pour ce trou → rien à mémoriser
      if (_geoPendingGap && _geoPendingGap.suggestedPillar) _geoPillarSel.value = _geoPendingGap.suggestedPillar;
    } else {
      ...
      _geoEmptyBtn.style.display = 'none';
      _geoSaveCheck.parentElement.style.display = '';
      _geo.prompted = true;
      saveGeoAnchor();
  ...
  document.getElementById('geo-confirm-btn').addEventListener('click', function() {
    ...
    if (_geoSaveCheck.checked && _pendingGeoLat !== null) {
      ...
      if (existing) { existing.name = name; existing.pillarId = pillar; locId = existing.id; }
      else {
        // visitCount part de 0 : la visite en cours est comptée par l'incrément de createGeoBlock()
        // juste après (donc 1 au final), pour ne pas compter deux fois la même confirmation.
        var newLoc = { id: generateId('loc'), name: name, lat: _pendingGeoLat, lng: _pendingGeoLng, pillarId: pillar, visitCount: 0 };
        state.knownLocations.push(newLoc);
        locId = newLoc.id;
      }
    }
    createGeoBlock(name, pillar, locId);
    closeGeoModal();
  });
  document.getElementById('geo-empty-btn').addEventListener('click', function() {
    resolveGapAsUnknown();
    closeGeoModal();
  });
  document.getElementById('geo-cancel-btn').addEventListener('click', function() {
    // En mode "gap", fermer sans répondre ne doit pas laisser un trou silencieux dans la
    // journée : on résout comme "inconnu" (voir demande explicite : toujours remplir 24h/24h).
    if (_geoModalMode === 'gap') { resolveGapAsUnknown(); }
    closeGeoModal();
  });
  ...
  function onGeoPosition(pos) {
    ...
    // Relevé trop imprécis pour être exploité : on l'ignore complètement, comme s'il n'était
    // jamais arrivé (ni _prevPos, ni _geo, ni aucun state touché).
    if (typeof accuracy === 'number' && accuracy > GEO_ACCURACY_IGNORE_THRESHOLD) return;
    ...
    // bandeau "lieu détecté" par-dessus, ou de ré-étirer le bloc déjà clôturé).
    if (_geoPendingGap) return;

    // Détecter un trou de suivi (app fermée/arrière-plan) avant de traiter cette position :
    // si un trou vient d'être détecté, le bandeau "gap" s'ouvre et le reste de cette position
    // (trajet, ancre, extension) sera réévalué au prochain check GPS une fois répondu.
    if (_fillLocationGap(now)) return;
    ...
    if (_prevPos !== null) {
      ...
      if (!_inTransit && moveDist > Math.max(300, accuracy * GEO_MOVE_ACCURACY_MARGIN) && moveMinutes <= 5) {
      ...
      if (_inTransit) {
        if (moveDist < 120) {
          ...
          if (_stableAtDest >= 3) {
            ...
            if (knownDest) {
              if (knownDest.visitCount >= TRUSTED_VISIT_THRESHOLD) { createGeoBlock(knownDest.name, knownDest.pillarId, knownDest.id); }
              else { openGeoModal(lat, lng, knownDest.name); }
            }
    ...
    if (distFromAnchor > Math.max(400, accuracy * GEO_MOVE_ACCURACY_MARGIN)) {
    ...
    if (secSinceAnchor >= 5 * 60 && !_geo.prompted) {
      ...
      if (known) {
        if (known.visitCount >= TRUSTED_VISIT_THRESHOLD) { createGeoBlock(known.name, known.pillarId, known.id); }
        else { openGeoModal(lat, lng, known.name); }
      }
      else { reverseGeocode(lat, lng, function(name) { openGeoModal(lat, lng, name); }); }
    }
  ...
  var _geoErrorShownDate = null; // n'afficher le bandeau d'erreur qu'une fois par jour, pas à chaque check
  function onGeoError(err) {
    // BUG FIX : avant ce correctif, TOUTE erreur GPS (permission refusée, position
    // indisponible, timeout) était totalement silencieuse — aucune trace, aucun signal.
    // Si la permission de localisation a été révoquée (fréquent sur iPhone après un moment,
    ...
    if (err && err.code === 1 /* PERMISSION_DENIED */) {
      ...
      _geoErrorShownDate = todayK;
      _geoPlaceHint.textContent = '🚫 Localisation refusée';
      _geoTimeHint.textContent = "Boussole n'a plus accès à ta position. Vérifie Réglages → Confidentialité → Localisation (ou Réglages → Safari → Boussole) et autorise l'accès.";
      _geoModalMode = 'live';
      _geoEmptyBtn.style.display = 'none';
      _geoSaveCheck.parentElement.style.display = 'none';
      _geoNameInput.style.display = 'none';
      document.getElementById('geo-pillar-select').style.display = 'none';
      document.getElementById('geo-confirm-btn').style.display = 'none';
      _geoBanner.style.display = 'block';
      sendNotif('🚫 Localisation refusée', "Boussole n'a plus accès à ta position — vérifie les réglages de l'app.");
    }
  }
  ...
  function startLocationTracking() {
    ...
    // appelés juste après startLocationTracking() en bas du fichier — d'où les prompts de
    // localisation ET de sommeil qui cessaient tous les deux en même temps.
    try { loadGeoAnchor(); } catch(e) {} // restaurer l'ancre depuis localStorage
    try { _extendOngoingBlock(); } catch(e) {} // mise à jour immédiate à l'ouverture (sans attendre GPS)
  ...
  document.getElementById('location-form-save').addEventListener('click', function(){
    ...
    if(id){
      ...
      // Ajout manuel = geste de confiance explicite : pas de période de confirmation à traverser.
      state.knownLocations.push({ id: generateId('loc'), name: name, lat: _locPending.lat, lng: _locPending.lng, pillarId: pillar, visitCount: TRUSTED_VISIT_THRESHOLD });
    }
    saveData();
    document.getElementById('location-form-card').style.display = 'none';
    renderLocationsList();
  ...
  // Chaque étape est isolée : avant ce correctif, une exception dans startLocationTracking()
  // empêchait requestNotifPermission() et ce premier checkSleepDetection() de s'exécuter,
  // ce qui coupait silencieusement les deux détections en même temps.
  try { startLocationTracking(); } catch(e) {}
  requestNotifPermission();
  try { checkSleepDetection(); } catch(e) {}
})();
